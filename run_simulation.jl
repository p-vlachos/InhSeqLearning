...
# random_seeds_p = [2061, 5987, 3642, 9465, 1837]
random_seeds_h = [6487, 3791, 6482, 6485, 9316]
# random_seeds_extra = [8174, 9729, 8537, 1835, 6935]

Nsimulations = length(random_seeds_h)			# Number of simulation to run
...
for sim_num = 1:Nsimulations
	...
	if loadnet
		...
		if stim_mode == "stimulation"
			...
			stim = makeStimSeq_brief(T, Npop=16, seq_len=4, seq_num=4, stim_rate=4., randomize=false)
		else
			# Spontaneous activity (no stimulation)
			stim = zeros(4, 16)
		end
	else
		# Full train 4 sequences x 4 assemblies
		T = 1_500_000
		stim = makeStimSeq(4, 20, seq_len=4)
	end
...
# 				stim = makeStimSeq_brief(T, Npop=12, seq_len=4, seq_num=3, stim_rate=4., randomize=false)	# Brief stimulation of 1st assembly in sequences
# 			else
