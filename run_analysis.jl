...
fitted_params = zeros(2, Npop, Nsims)
...
for sim_num = 1:Nsims
    ...
    for ipop = 1:Npop
        ...
        fitted_params[:, ipop, sim_num] .= power_fit(x_vals, sorted_ie_weights[:, ipop, sim_num])
...
ax = CairoMakie.Axis(f[1, 1], xlabel=L"I_2 \, \text{neuron index}", ylabel=L"\text{Synaptic strength (sum; pF)}",
                    ...
                    xticks=([0, 27, 50, 100, 150, 200, 250], [L"0", L"\mathbf{27}", L"50", L"100", L"150", L"200", L"250"]),
                    xlabelsize=24, ylabelsize=24, xticklabelsize=18, yticklabelsize=18, limits=(0., 250.5, 0.5, 249))
CairoMakie.band!(x_vals, plot_data_band_min, plot_data_band_max)
CairoMakie.lines!(x_vals, mean(fitted_params[1, :, :]) .* (x_vals .^ mean(fitted_params[2, :, :])), linewidth=4, color=:gold3, label=L"y = %$(round(mean(fitted_params[1, :, :]), digits=2))\,(\pm %$(round(std(fitted_params[1, :, :]), digits=2)))\,x^{%$(round(mean(fitted_params[2, :, :]), digits=2))\,(\pm %$(round(std(fitted_params[2, :, :]), digits=2)))}")
